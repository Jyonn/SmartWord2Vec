...
class SmartPrinter:
    ...
    def __getitem__(self, prefix):
        ...
        return SmartPrinter.create(prefixes=prefixes)

    def get_prefix_strings(self):
        return [prefix.get_string() for prefix in self.prefixes]

    def get_colored_prefix_strings(self):
        return [prefix.colored_string(SmartPrinter.prefix_color) for prefix in self.prefixes]
    ...
    def __call__(self, *args):
        ...
        if SmartPrinter.logger:
            SmartPrinter.logger(' '.join([prefix_string, display_string]))
